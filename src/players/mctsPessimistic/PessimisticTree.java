...
import players.heuristics.PessimisticHeuristics;
...
public class PessimisticTree {
    ...
    private players.mctsPessimistic.PessimisticTree parent;
    private players.mctsPessimistic.PessimisticTree[] children;
    ...
    PessimisticTree(pessimisticParams p, Random rnd, int num_actions, Types.ACTIONS[] actions) {
        this(p, null, -1, rnd, num_actions, actions, 0, null);
    }

    private PessimisticTree(pessimisticParams p, players.mctsPessimistic.PessimisticTree parent, int childIdx, Random rnd, int num_actions,
        ...
        children = new players.mctsPessimistic.PessimisticTree[num_actions];
        totValue = 0.0;
    ...
    void setRootGameState(GameState gs)
    {
        ...
        else if (params.heuristic_method == params.PESSIMISTIC_HEURISTIC)
            this.rootStateHeuristic = new PessimisticHeuristics(gs, m_rnd);
    ...
    void mctsSearch(ElapsedCpuTimer elapsedTimer) {
        ...
        while(!stop){
            ...
            PessimisticTree selected = treePolicy(state);
    ...
    private PessimisticTree treePolicy(GameState state) {

        PessimisticTree cur = this;
        GameState[] boards;

        while (!state.isTerminal() && cur.m_depth < params.rollout_depth)
        {
            if (cur.m_depth == params.pessimistic_depth) {
                boards = PessimisticSimulation(state);
                evaluate_boards(boards);
            }
    ...
    public GameState[] PessimisticSimulation(GameState state) {
        GameState[] boards = new GameState[params.pessimistic_length];
        boards[0] = state.copy();

        for (int i = 1; i < params.pessimistic_length; i++) {
            int action = safeRandomAction(state);
            roll(state, actions[action]);
            boards[i] = state.copy();
        }

        for (int i = 1; i < params.pessimism; i++) {
            for (GameState gs : boards) {
                int action = safeRandomAction(gs);
                possiblePositions(gs, actions[action]);
            }
        }
    ...
    public void evaluate_boards(GameState[] boards) {
        for (GameState gs : boards) {
            this.totValue += rollOut(gs);
        }
    }

    private PessimisticTree expand(GameState state) {
        ...
        PessimisticTree tn = new PessimisticTree(params,this,bestAction,this.m_rnd,num_actions,
                actions, fmCallsCount, rootStateHeuristic);
        children[bestAction] = tn;
        return tn;
    }

    private void possiblePositions(GameState state, Types.ACTIONS a) {
        ...
        for(int i = 0; i < nPlayers; ++i)
        {
            if(playerId == i)
            {
                ...
                int actionIdx = m_rnd.nextInt(state.nActions());
    ...
    private PessimisticTree uct(GameState state) {
        PessimisticTree selected = null;
        double bestValue = -Double.MAX_VALUE;
        for (PessimisticTree child : this.children)
        {
    ...
    private void backUp(PessimisticTree node, double result)
    {
        PessimisticTree n = node;
        while(n != null)
        {
            n.nVisits++;
            n.totValue += result;
    ...
    private boolean notFullyExpanded() {
        for (PessimisticTree tn : children) {
            if (tn == null) {
                return true;
            }
        }
